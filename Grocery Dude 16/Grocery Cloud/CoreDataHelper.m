...
- (id)init {
    ...
    SM_CACHE_ENABLED = YES; // Use StackMob Cache Store in case network is unavailable
    SM_CORE_DATA_DEBUG = NO; // Log loads of stuff to the console
    
#warning Enter your own publicKey here. Read Learning Core Data by Tim Roadley for more information.
    _stackMobClient  = [[SMClient alloc] initWithAPIVersion:@"0" publicKey:@"c2804333-3928-4d59-923f-cb15210a62e8"]; // APIVersion 0 = Dev, 1 = Prod
...
- (void)saveContext {
    ...
    if (!error) {
        NSLog(@"StackMob context SAVED changes to StackMob store (in the foreground)");
    } else {
        NSLog(@"StackMob context FAILED to save changes to StackMob store (in the foreground): %@", error);
    }
}
- (void)backgroundSaveContext {
    ...
    [stackMobContext saveOnSuccess:^{
        NSLog(@"StackMob context SAVED changes to StackMob store (in the background)");
    } onFailure:^(NSError *error) {
        NSLog(@"StackMob context FAILED to save changes to StackMob store (in the background): %@", error);
    }];
}
...
#pragma mark - UNIQUE ATTIRIBUTE SELECTION (This code is Grocery Cloud data specific and is used when instantiating CoreDataImporter)
