...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.frc.managedObjectContext performBlock:^{
        if (!item.storedIn ||
            ...
            NSLog(@"sectionNameKeyPath WORKAROUND (See Appendix B in ‘Learning Core Data’ by Tim Roadley: item.storedIn is now = '%@'", 	item.storedIn);
        }
        if (!item.aisle ||
            ![item.aisle isEqualToString:item.locationAtShop.aisle]) {
            item.aisle = item.locationAtShop.aisle;
            NSLog(@"sectionNameKeyPath WORKAROUND (See Appendix B in ‘Learning Core Data’ by Tim Roadley: item.aisle is now = '%@'", item.aisle);
        }
    }];
    return cell;
}
